...
export function adaptSeerResponse(seerData: any): SeerKundli {
  ...
  // Use Seer's original house values (Whole-Sign / Rashi houses).
  // Seer assigns houses based on sign: House 1 = Ascendant sign, House 2 = next sign, etc.
  // This matches the D1 chart image from the Seer Chart API.
  console.log(`✅ [SEER] Adaptation complete: ${planets.length} planets + Asc (using Seer whole-sign houses)`);
  ...
  return {
    ...
    notes: [
      ...
      "Houses: Whole-Sign (Rashi = House), matching Seer chart images",
      ...warnings
    ]
  };
}

/// Helper: Calculate house from reference sign index (Whole Sign — kept for backward compat)
export function houseFrom(refSignIdx: number, targetSignIdx: number): number {
  return ((targetSignIdx - refSignIdx + 12) % 12) + 1;
}

// Bhava Chalit (Equal House): house based on degree distance from Ascendant.
// Asc degree = start of 1st house. Each house spans exactly 30°.
// This matches the D1 chart from the Seer Chart API.
export function bhavaCharitHouse(planetDeg: number, ascDeg: number): number {
  const normalized = ((planetDeg - ascDeg + 360) % 360);
...
// Degrees are interpolated linearly. Houses are taken from the Seer snapshots
// directly (when both snapshots agree) to avoid errors from non-linear ascendant
// movement. Only when snapshots disagree on a house do we fall back to
// degree-based Whole-Sign calculation.
