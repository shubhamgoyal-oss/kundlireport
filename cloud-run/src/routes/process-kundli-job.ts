...
import { fetchSeerKundli, adaptSeerResponse, interpolateKundli, nextHourParams, type SeerKundli, type SeerKundliRequest } from '../_shared/generate-kundli-report/seer-adapter';
import { generatePanchangPrediction } from '../_shared/generate-kundli-report/panchang-agent';
import { generatePillarsPrediction } from '../_shared/generate-kundli-report/pillars-agent';
import { generateAllPlanetProfiles } from '../_shared/generate-kundli-report/planets-agent';
import { generateAllHouseAnalyses } from '../_shared/generate-kundli-report/houses-agent';
import { generateCareerPrediction, type CareerPrediction } from '../_shared/generate-kundli-report/career-agent';
import { generateMarriagePrediction, type MarriagePrediction } from '../_shared/generate-kundli-report/marriage-agent';
import { generateDashaMahadashaPrediction, generateDashaAntardashaPrediction, mergeDashaResults, type DashaPrediction } from '../_shared/generate-kundli-report/dasha-agent';
import { generateRahuKetuPrediction } from '../_shared/generate-kundli-report/rahu-ketu-agent';
import { generateRemediesPrediction, type RemediesPrediction } from '../_shared/generate-kundli-report/remedies-agent';
import { generateNumerologyPrediction } from '../_shared/generate-kundli-report/numerology-agent';
import { generateSpiritualPrediction } from '../_shared/generate-kundli-report/spiritual-agent';
import { generateCharaKarakasPrediction } from '../_shared/generate-kundli-report/chara-karakas-agent';
import { generateGlossaryPrediction } from '../_shared/generate-kundli-report/glossary-agent';
import { generateDoshasPrediction } from '../_shared/generate-kundli-report/doshas-agent';
import { generateRajYogsPrediction } from '../_shared/generate-kundli-report/raj-yogs-agent';
import { generateSadeSatiPrediction } from '../_shared/generate-kundli-report/sade-sati-agent';
import { enforceAstrologyTruth } from '../_shared/generate-kundli-report/truth-guard';
import { setAgentLanguageContext, setAgentNativeContext, getAgentLanguage } from '../_shared/generate-kundli-report/agent-base';
import { getLanguagePack, normalizeLanguage } from '../_shared/language-packs/index';
import type { SupportedLanguage } from '../_shared/language-packs/types';
import { safetyText } from '../_shared/safety-i18n';
import { createJobEvent, touchJobHeartbeat } from '../_shared/job-events';
import { insertKundliApiCall, insertKundliApiCalls } from '../_shared/kundli-audit';

import { calculateCharaKarakas } from '../_shared/generate-kundli-report/utils/chara-karakas';
import { calculateAspects, getConjunctions } from '../_shared/generate-kundli-report/utils/aspects';
import { calculateTithi } from '../_shared/generate-kundli-report/utils/panchang';
import { getSignLord } from '../_shared/generate-kundli-report/utils/dignity';
 ...
 */
function triggerStage2AndReturn(
  supabase: any,
  jobId: string,
  res: Response,
): void {
  ...
  // Fire and forget — no EdgeRuntime.waitUntil needed in Express
  stage2Promise;

  console.log(`✅ [RETRY] Stage 2 (translate) triggered for job ${jobId}`);
  res.json({ success: true, jobId, stage: "retry_complete" });
}
...
router.post('/', async (req: Request, res: Response) => {
  ...
  // Safety timeout: save + trigger Stage 2 BEFORE the function is killed.
  ...
  try {
    ...
    // save, and trigger Stage 2. Gets its own fresh time budget.
    // ──────────────────────────────────────────────────────────────────────────
    if (retryPass >= 2) {
      await handleAgentRetry(supabaseAdmin, job, res);
      return;
    }
    ...
    // Race against a safety timeout so we save partial results before being killed.
    ...
    if (needsRetry) {
      ...
      const retryUrl = `http://localhost:${process.env.PORT || 8080}/process-kundli-job`;
      ...
      return res.json({ success: true, jobId, stage: "generate_partial_retry_queued", failedAgents: timedOutAgents });
    }
    ...
    return res.json({ success: true, jobId, stage: "generate_complete" });
  } catch (error) {
    ...
    if (jobId) {
      ...
      if (partialReport) {
        ...
        try {
          ...
          return res.json({ success: true, jobId, stage: "generate_partial", error: errMsg });
        } catch (saveErr) {
