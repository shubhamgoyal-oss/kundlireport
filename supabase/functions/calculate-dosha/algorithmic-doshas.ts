...
export function calculateSadeSatiAlgorithmic(
  ...
  // Try to use shadhe_sati_dosha events first
  ...
  if (events && Array.isArray(events) && events.length > 0) {
    ...
    return {
      ...
      source: "events",
      window: null,
      reasons: ["Not currently in Sade Sati period"],
      debug: { windows, moonSign: moon.sign }
    };
  }

  // Fallback: Use current Saturn position
  // NOTE: This requires periodic updates to CURRENT_SATURN_SIGN
  ...
  if (!saturn) {
    return {
      ...
      source: "current_saturn",
      window: null,
      reasons: ["Saturn position not available"],
      debug: {}
    };
  }

  // Check if Saturn is in Moon-1, Moon, or Moon+1
  const saturnRelToMoon = (saturn.signIdx - moon.signIdx + 12) % 12;
  
  if (saturnRelToMoon === 11) {
    // Saturn is 1 sign before Moon (12th from Moon = Moon-1)
    return {
      active: true,
      phase: "rising",
      source: "current_saturn",
      window: null,
      reasons: [`Saturn in ${saturn.sign}, Moon in ${moon.sign} (Rising phase)`],
      debug: { saturnSign: saturn.sign, moonSign: moon.sign, saturnRelToMoon }
    };
    ...
    return {
      ...
      source: "current_saturn",
      window: null,
      reasons: [`Saturn in ${saturn.sign}, Moon in ${moon.sign} (Peak phase)`],
      debug: { saturnSign: saturn.sign, moonSign: moon.sign, saturnRelToMoon }
    };
  } else if (saturnRelToMoon === 1) {
    // Saturn is 1 sign after Moon
    return {
      active: true,
      phase: "setting",
      source: "current_saturn",
      window: null,
      reasons: [`Saturn in ${saturn.sign}, Moon in ${moon.sign} (Setting phase)`],
      debug: { saturnSign: saturn.sign, moonSign: moon.sign, saturnRelToMoon }
    };
  }
  ...
  return {
    ...
    source: "current_saturn",
    window: null,
    reasons: ["Saturn not in Sade Sati position relative to Moon"],
    debug: { saturnSign: saturn.sign, moonSign: moon.sign, saturnRelToMoon }
  };
}
