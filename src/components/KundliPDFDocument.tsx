...
const MONTH_NAMES_BY_LANGUAGE: Record<'en' | 'hi' | 'te' | 'kn' | 'mr', string[]> = {
...
let ACTIVE_PDF_LANGUAGE: 'en' | 'hi' | 'te' | 'kn' | 'mr' = 'en';

const PDF_UI_PHRASE_MAP: Record<string, Record<string, string>> = {
  hi: {
...
const PDF_UI_WORD_MAP: Record<string, Record<string, string>> = {
  hi: {
    Report: 'रिपोर्ट',
    Birth: 'जन्म',
    Details: 'विवरण',
...
const DISCLAIMER_CONTENT: Record<string, { title: string; paragraphs: string[] }> = {
  en: {
    title: 'Disclaimer',
    paragraphs: [
...
const GUIDANCE_CONTENT: Record<string, { title: string; paragraphs: string[] }> = {
  en: {
...
const localizePdfUiText = (raw: string | null | undefined): string => {
  ...
  // When text is predominantly in non-Latin script (Hindi/Telugu/Kannada/Marathi),
  // strip remaining isolated English words the maps didn't catch.
  // This ensures near-zero English leakage in translated content.
  if (output.length > 10) {
    const letters = output.replace(/[\s\d\p{P}\p{S}]/gu, '');
    if (letters.length > 0) {
      const latinCount = (letters.match(/[A-Za-z]/g) || []).length;
      // Only sweep if text is >60% non-Latin (i.e., <40% Latin)
      if (latinCount > 0 && latinCount / letters.length < 0.4) {
        // Strip isolated English words (3+ chars) — preserves short ones like AM, PM, OK
        const cleaned = output.replace(/\b[A-Za-z]{3,}\b/g, '');
        // Only apply if we didn't destroy the entire text
...
export const KundliPDFDocument = ({ report, language }: KundliPDFProps) => {
  ...
  const parsePlaceDetails = (place: string, fallback: any) => {
    ...
    const latLonFallback = hasCoords
      ...
      : (ACTIVE_PDF_LANGUAGE === 'hi' ? 'उपलब्ध नहीं' : ACTIVE_PDF_LANGUAGE === 'te' ? 'అందుబాటులో లేదు' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಲಭ್ಯವಿಲ್ಲ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'उपलब्ध नाही' : 'N/A');
  ...
  const sex = genderRaw === 'F' || genderRaw === 'FEMALE'
    ? (ACTIVE_PDF_LANGUAGE === 'hi' ? 'महिला' : ACTIVE_PDF_LANGUAGE === 'te' ? 'స్త్రీ' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಮಹಿಳೆ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'स्त्री' : 'Female')
    : genderRaw === 'O' || genderRaw === 'OTHER'
      ? (ACTIVE_PDF_LANGUAGE === 'hi' ? 'अन्य' : ACTIVE_PDF_LANGUAGE === 'te' ? 'ఇతర' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಇತರ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'इतर' : 'Other')
      : genderRaw === 'M' || genderRaw === 'MALE'
        ? (ACTIVE_PDF_LANGUAGE === 'hi' ? 'पुरुष' : ACTIVE_PDF_LANGUAGE === 'te' ? 'పురుషుడు' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಪುರುಷ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'पुरुष' : 'Male')
        : (ACTIVE_PDF_LANGUAGE === 'hi' ? 'उपलब्ध नहीं' : ACTIVE_PDF_LANGUAGE === 'te' ? 'లభ్యం కాదు' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಲಭ್ಯವಿಲ್ಲ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'उपलब्ध नाही' : 'N/A');
    ...
    <Document>
        ...
        <View style={styles.coverIdentityCard}>
          ...
          <View style={styles.coverInfoBlock}>
            ...
            <View style={[styles.coverInfoRow, { marginBottom: 0 }]}>
              ...
              <Text style={styles.coverInfoValue}>{placeDetails.city || report.birthDetails.placeOfBirth || (ACTIVE_PDF_LANGUAGE === 'hi' ? 'उपलब्ध नहीं' : ACTIVE_PDF_LANGUAGE === 'te' ? 'అందుబాటులో లేదు' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಲಭ್ಯವಿಲ್ಲ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'उपलब्ध नाही' : 'N/A')}</Text>
            </View>
          </View>
        </View>
      ...
      {charts.length > 0 && (() => {
        ...
        const chartName = (c: ChartData) =>
          ...
          : ACTIVE_PDF_LANGUAGE === 'kn' && c.nameKannada ? c.nameKannada
          : ACTIVE_PDF_LANGUAGE === 'mr' && c.nameMarathi ? c.nameMarathi
          : c.name;
            ...
            <View style={styles.stableTwoCol}>
              <View style={styles.stableCol}>
                ...
                <InfoRow label="Place of Birth" value={(() => {
                  ...
                  if (!raw) return placeDetails.city || (ACTIVE_PDF_LANGUAGE === 'hi' ? 'उपलब्ध नहीं' : ACTIVE_PDF_LANGUAGE === 'te' ? 'అందుబాటులో లేదు' : ACTIVE_PDF_LANGUAGE === 'kn' ? 'ಲಭ್ಯವಿಲ್ಲ' : ACTIVE_PDF_LANGUAGE === 'mr' ? 'उपलब्ध नाही' : 'N/A');
