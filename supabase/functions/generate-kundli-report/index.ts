...
function enforceGlobalPredictionSafety(
  ...
  safeReport.career = enforceCareerSafety(safeReport.career, birthDate, referenceDate);
  safeReport.marriage = enforceMarriageSafety(safeReport.marriage, maritalStatus, birthDate, referenceDate);
  safeReport.dasha = enforceDashaSafety(safeReport.dasha, birthDate);
...
serve(async (req) => {
  ...
  try {
    ...
    const baseReq: SeerKundliRequest = {
      ...
      hour: istHour,
      min: 0,               // force to :00 (Seer ignores min field)
      lat: latitude,
      lon: longitude,
    ...
    if (Math.floor(istMin) === 0) {
      ...
      // Need interpolation: fetch H:00 and (H+1):00 IST in parallel
