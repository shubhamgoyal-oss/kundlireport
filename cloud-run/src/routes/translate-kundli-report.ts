 ...
 * Gets its OWN wall_clock_limit dedicated entirely to translation.
 * After completing, triggers finalize-kundli-report (Stage 3).
 *
 ...
 * again for the remaining strings.
 *
...
import { runTranslationSweep } from '../_shared/generate-kundli-report/translation-agent';
import { setAgentLanguageContext } from '../_shared/generate-kundli-report/agent-base';
import { normalizeLanguage } from '../_shared/language-packs/index';
...
router.post('/', async (req: Request, res: Response) => {
  ...
  // Safety timeout: trigger Stage 3 before we run too long.
  ...
  try {
    ...
    if (needsSecondPass) {
      ...
      return res.json({ success: true, jobId, stage: `translate_pass_${translationPass}_complete`, nextPass: translationPass + 1 });
    }
    ...
    return res.json({ success: true, jobId, stage: "translate_complete" });
  } catch (error) {
    ...
    if (jobId) {
      ...
      return res.json({ success: true, jobId, stage: "translate_error_skipped", error: errMsg });
    }
...
async function saveAndTriggerSelf(
  ...
  try {
    ...
    const selfUrl = `http://localhost:${process.env.PORT || 8080}/translate-kundli-report`;
    fetch(selfUrl, {
      method: "POST",
      headers: {
